...
#pragma omp parallel for private(i,j) shared (m,n)
  for ( i = 0; i < m; i++ )
  {
    x = ( ( double ) (     i     ) * x_max
        + ( double ) ( m - i - 1 ) * x_min )
        / ( double ) ( m     - 1 );
    // #pragma omp parallel for private (j) shared (n)
    for ( j = 0; j < n; j++ )
    {
      y = ( ( double ) (     j     ) * y_max
          + ( double ) ( n - j - 1 ) * y_min )
...
*/
{
  ...
  // #pragma omp parallel for private (k) shared (count_max)
...
*/
  ...
  g_index = g;
  b_index = b;
  #pragma omp prallel for private (i,j) shared (xsize,ysize)
  // #pragma omp prallel for private (j) shared (ysize)
  for ( j = 0; j < ysize; j++ )
  {
    // #pragma omp parallel for private (i) shared (xsize)
    for ( i = 0; i < xsize; i++ )
    {
      // #pragma omp critical
      if ( rgb_max < *r_index )
...
*/
{
  ...
  // #pragma omp parallel for private (j) shared (ysize)
  for ( j = 0; j < ysize; j++ )
  {
    // #pragma omp parallel for private (i) shared (xsize)
    for ( i = 0; i < xsize; i++ )
